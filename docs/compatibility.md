...
| `lerobot-eval` | unimplemented | `lerobot.scripts.lerobot_eval:main` | Evaluate a policy by running environment rollouts. | Local ACT checkpoint inference is ported, but this executable still needs Gymnasium environment integration and evaluation metric/reporting semantics; fabricating metrics would be worse than failing. |
...
| `lerobot-rollout` | partial | `lerobot.scripts.lerobot_rollout:main` | Run a trained policy on a real robot with pluggable strategies. | Runnable for a hardware-independent local ACT deployment and a finite, state-only SO-101 follower deployment: the local path loads checkpoint processor state and dataset observations; the hardware path loads upstream calibration JSON, pings and validates all six Feetech IDs, uses one sync-read position request, converts calibrated state units, runs a local ACT checkpoint, writes six finite position goals only after explicit confirmation, paces each control tick at the positive finite `--fps` target (default 30 Hz), and disables torque on every exit. A protocol-valid mock transport now exercises the real policy/handshake/control/cleanup boundary in CI. The library deployment seam also accepts caller-owned single-observation batches; its episode-aware stream accepts explicit reset markers before observations so queued ACT actions cannot cross simulator episode boundaries. Physical serial hardware and the broader robot/environment strategy surface remain unexercised. Cameras, teleoperators, environments, visualization, video shards, and other rollout strategies remain explicitly refused. |
...
| `lerobot/configs` | partial | 11 | `configs.types` str-enums and `PolicyFeature` are ported and tested. The ACT policy's concrete config is too, including the `from_pretrained`/`_save_pretrained` checkpoint JSON path and the Draccus value conversions it decodes through. The full Draccus CLI parser is not, but `lerobot-train` now preserves the native JSON `train_config.json` path and its ordered `rename_map` field while consuming an explicit allow-list of flags and refusing everything else by name. |
...
| `lerobot/robots` | partial | 53 | SO-101 low-level Feetech follower transport, calibration loading, model-checked discovery, sync-read state conversion, and finite action writes are ported; SO-101 cameras, teleoperation, and all other robot families remain hardware-gated or unimplemented. |
...
| `lerobot/scripts` | partial | 20 | `lerobot_info`, `lerobot_train`, and the local ACT paths of `lerobot_rollout` (dataset-backed and finite, state-only calibrated SO-101) are ported and runnable. The other 15 entry points exist only as executables that fail with a stable unsupported error. |
...
| `rerobot_core::processor::pipeline` | `lerobot/processor/pipeline.py` (`DataProcessorPipeline` ordered forwarding, stateless reset, feature transforms, registry-first config resolution, and config serialization for the native JSON subset) | `crates/rerobot-core/tests/processor_pipeline.rs` |
...
| `rerobot_train::processor` | the four pre/postprocessor artifacts `save_checkpoint` writes, from `lerobot/policies/factory.py`'s `make_pre_post_processors` and `lerobot/processor/pipeline.py`'s `_save_pretrained`; the saved ACT preprocessor rename map plus scalar/camera normalizer and action unnormalizer state consumed by the local deployment boundary | `crates/rerobot-train/tests/processor.rs`, byte for byte against upstream's own output, plus runtime scalar/camera rename-normalization tests and `crates/rerobot-train/tests/deploy.rs`'s changed-dataset-statistics differential. Non-empty `artifacts` declarations introduced by current upstream are rejected explicitly until native artifact resolution is implemented; they are never silently dropped. |
...
| `rerobot_train::deploy` | `lerobot/policies/act/modeling_act.py`'s `select_action` queue and `ACTTemporalEnsembler`, plus the local observation side of `lerobot/scripts/lerobot_rollout.py` | `crates/rerobot-train/tests/deploy.rs`, `crates/rerobot-cli/tests/rollout_cli.rs`; checkpoint-only caller batches can be streamed through `rollout_batches_with_sink` with the same preprocessing and an explicit episode-reset boundary |
...
| Building the interpolated sequence for a huge multiplier ends in `MemoryError` after CPython grinds through the `list` | `ActionInterpolator::add` returns `Err(InterpolatorError::BufferNotAllocatable)` up front, including when the multiplier exceeds `MAX_INTERPOLATION_STEPS` (1,000,000) or the output grid exceeds `MAX_INTERPOLATION_ELEMENTS` (16,777,216 scalars) | The sequence is `multiplier` elements of per-step Rust vectors, so the port refuses explicit resource boundaries before asking the allocator to reserve a hostile amount rather than truncating the step count. Below those boundaries the buffer is genuinely built; an allocator that fails part-way remains a process-level boundary where CPython would raise `MemoryError`. |
...
| Upstream registers the step as `smolvla_new_line_processor` and reconstructs it through the processor registry | Exposes that exact spelling and reconstructs it through the native JSON pipeline registry | Dynamic custom registration, class imports, tensor-state loading, and pipeline steps outside the native stateless subset remain unsupported; unsupported entries fail closed. |
...
* `RenameObservationsProcessorStep`: the rename map is applied exactly once per
  ...
  kept). The pinned 0.6.1 source was exact-key only. Current upstream `main`
  (`3f2c29ef7e44b1ddccbcda3b6a63939e53639e9e`) now carries a base feature rename
  through the derived `_is_pad` and `_padding_mask` keys at the runtime observation
  and ACT batch boundaries, which Rerobot includes as a forward-compatible
  extension; the separate feature-declaration transform remains exact-key-only.
  An explicit mapping for a derived key still wins.
