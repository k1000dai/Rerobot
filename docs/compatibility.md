...
| `lerobot/configs` | partial | 11 | `configs.types` str-enums and `PolicyFeature` are ported and tested. The ACT policy's concrete config and checkpoint JSON shape are also ported; the generic Draccus CLI parser and train/eval configs are not. |
...
| `lerobot/policies` | partial | 128 | ACTConfig validation, presets, delta indices and checkpoint JSON are ported. The ACT processor and tensor model, and every other policy architecture, are not. |
...
| `rerobot_core::policy::act` | `lerobot/policies/act/configuration_act.py` (`ACTConfig` only) | `crates/rerobot-core/tests/act_config.rs` |
...
| CPython's JSON writer emits bare `NaN`/`Infinity` tokens for non-finite ACT float fields | ACT config serialization returns an explicit error for a non-finite float | Those tokens are accepted extensions, not standard JSON, and `serde_json` would otherwise silently emit `null`. Finite floats, numeric strings, booleans, and integer/float JSON numbers follow Draccus' coercion; non-finite checkpoint wire parity remains unported rather than corrupted. |
