...
| `rerobot_train::processor` | the four pre/postprocessor artifacts `save_checkpoint` writes, from `lerobot/policies/factory.py`'s `make_pre_post_processors` and `lerobot/processor/pipeline.py`'s `_save_pretrained`; the saved ACT preprocessor rename map plus normalizer/unnormalizer state consumed by the local deployment boundary | `crates/rerobot-train/tests/processor.rs`, byte for byte against upstream's own output, plus the runtime rename/normalization test and `crates/rerobot-train/tests/deploy.rs`'s changed-dataset-statistics differential |
